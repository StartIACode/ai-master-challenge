...
**Controle negativo** (`negative_control`): o pipeline que classifica o Dataset 2, prevendo `Ticket Type` a partir de `Ticket Description` (5-fold, seed 42), acerta 19,7% ± 0,6 (folds entre 19,1% e 20,6%), abaixo da classe majoritária (Refund request, 20,7%): não há texto para aprender. No Dataset 2 o mesmo pipeline dá 84,4% (± 0,37 p.p.; `cv5`). O Dataset 1 ainda dá Refund, Cancellation e Billing como exemplos do que **não** automatizar.

## 3. Dataset 2: onde a triagem acerta, onde erra e quanto o erro custa

**Idioma** (etapa da rodada 2): o Dataset 2 é em inglês e já vem pré-processado (minúsculas, sem pontuação, sem stopwords). Para o protótipo operar em português, o corpus foi traduzido uma única vez para pt-BR com tradutor offline (Argos Translate / CTranslate2, pacote en_pt 1.9; `prototipo/scripts/traduzir_ds2.py`; metadados em `prototipo/data/ds2_pt.meta.json`), com rótulos inalterados e termos de produto (Windows, Outlook, VPN…) preservados. A tradução é telegráfica (o texto-fonte não tem pontuação nem stopwords) e mistura formas pt-PT e pt-BR; o retreino custou ~2 p.p.: 86,5% de acurácia em inglês → 84,5% em português. Em produção o modelo é treinado no histórico em português da própria empresa, sem tradução.

**Mix** (`class_counts`; 436 quase-duplicatas removidas; 37.920 tickets de treino, 9.481 de hold-out): Hardware 13.573 · HR Support 10.797 · Access 7.098 · Miscellaneous 7.024 · Storage 2.772 · Purchase 2.275 · Internal Project 2.106 · Administrative rights 1.756. **Modelo**: TF-IDF (154.859 termos) + regressão logística; acurácia 84,5%, macro-F1 0,849, ECE 0,035 (`prototipo/artifacts/figures/reliability.png`); treino em 109 s, 1,4 GB de pico.

**Onde erra** (`per_class`, `confusion`; `prototipo/artifacts/figures/confusion.png`): Hardware absorve as confusões (precisão 0,79, recall 0,89; 650 falsos em 3.064 (21,2%) previstos). Administrative rights é a classe que o modelo menos reconhece (recall 0,67, precisão 0,88): 85 de 351 (24,2%) viram Hardware, e é elevação de privilégio. HR Support → Hardware: 207 de 2.160 (9,6%); Miscellaneous → Hardware: 150 de 1.405 (10,7%). Purchase (precisão 0,97), Internal Project (0,93), Storage (0,92) e Access (0,91) são as classes mais limpas.

**Quanto custa o erro** (`thresholds`; `prototipo/artifacts/figures/coverage_accuracy.png`): com limiar 0,90 o modelo cobre 49,3% dos tickets com 98,1% de acerto (87 erros em 4.675); aplicada a política (só Access, Storage e Hardware auto-roteiam, nenhum ticket com sinal de risco), a **cobertura útil é 24,8% com 98,0% de acerto: 47 erros em 2.352 tickets**, cada um devolvido pelo botão "IA errou" ao custo de uma re-triagem. O outro lado pesa mais: os 37,8% do hold-out com confiança abaixo de 0,80 (3.582 tickets) acertam só 65,0% e vão sempre a uma pessoa. Miscellaneous com p ≥ 0,90 seria 5,9% do hold-out "roteado com confiança para lugar nenhum"; nunca conta como roteado.

**Similaridade** (`similarity`; `prototipo/artifacts/figures/similarity_curve.png`): o vizinho mais parecido do treino tem a mesma classe em 68,3% dos casos, contra 84,5% do classificador; ≥ 0,90 cobre 1,2% do hold-out (114 tickets, 93,9% de concordância), ≥ 0,99 cobre 0,1% (6 tickets: templates e alertas). "Responder com a tratativa do ticket 90% parecido" alcançaria cerca de 1 ticket em 80.

## 4. O que exportar do helpdesk para medir em duas semanas

Log de auditoria que o helpdesk já grava, mais uma amostra cronometrada: (1) **eventos de status com carimbo de hora** (aberto, primeira resposta, aguardando cliente, resolvido, fechado, reaberto); (2) **fila e cada reatribuição**: o re-roteamento humano atual é o baseline do gate; (3) **categoria inicial e final**, mais 200 tickets rotulados por 2 triadores com o kappa registrado; (4) **nível que resolveu** (N1/N2/N3); (5) **tempo de trabalho ativo**, cronometrado por 5 agentes por 2 semanas (carimbos medem espera, não esforço); (6) **fechamento padronizado** (categoria, causa raiz, ação, reutilizável?, reaberto?, CSAT por nível). Sem isso, "tempo perdido" é premissa, e está rotulado assim na proposta. O TMA por nível que o protótipo mede no board é o da sessão; o histórico só vem do item (1).
